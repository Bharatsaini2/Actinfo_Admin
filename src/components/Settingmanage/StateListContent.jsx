...
const StateListContent = () => {
    ...
    const fetchStates = async (pageNumber = page, searchQuery = search) => {
        try {
            const res = await getData(`/state?page=${pageNumber}&search=${searchQuery}`);
    ...
    const handleDelete = (catId) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await deleteData(`/state/${catId}`,);
                    if (res.status_code === 200) {
                        toast.success(res.message);
                        await fetchStates();
                    }
                } catch (error) {
                    console.log("error delete states", error)
                }
            }
        });
    }
                    ...
                    <div className="card-body table-responsive">
                        <table className="table table-hover table-bordered">
                            ...
                            <tbody>
                                {
                                    stateList?.length > 0 ? (
                                        stateList?.map((age, index) => {
                                            return (
                                                <tr key={age.id}>
                                                    <td>{(page - 1) * 10 + index + 1}</td>
                                                    {/* <td>{age?.country_code}</td> */}
                                                    <td>{age?.state_name}</td>
                                                    {/* <td>
                                                    ...
                                                    <td className="text-end">
                                                        <div className="d-flex justify-content-end gap-2">
                                                            <Link to={`/state/view/${age.id}`} className="btn btn-sm btn-light"><FiEye /></Link>
                                                            <Link to={`/state/update/${age.id}`} className="btn btn-sm btn-light"><FiEdit3 /></Link>
...
export default StateListContent